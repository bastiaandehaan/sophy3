...
def parse_args():
    ...
    # Data argumenten
    parser.add_argument("--symbol", type=str, default="EURUSD", help="Trading symbool (bijv. EURUSD)")
    ...
    parser.add_argument("--bars", type=int, default=1000, help="Aantal bars voor backtesting")
    ...
    parser.add_argument("--parameter-preset", type=str, choices=["forex", "crypto", "index", "stock"],
    ...
    parser.add_argument("--volatility-factor", type=float, help="Volatiliteit factor voor filter")

    # Risk parameters
    parser.add_argument("--capital", type=float, default=10000, help="Initieel kapitaal")
    parser.add_argument("--risk", type=float, default=0.01, help="Risico per trade (bijv. 0.01 voor 1%)")
    ...
    parser.add_argument("--save-results", action="store_true", help="Sla resultaten op naar CSV")
    parser.add_argument("--output-dir", type=str, default="./results", help="Output directory voor resultaten")

    # Optimalisatie
    parser.add_argument("--optimize", action="store_true", help="Voer parameteroptimalisatie uit")
    parser.add_argument("--optimize-param", type=str, choices=["ema", "rsi", "volatility"],
                       help="Parameter om te optimaliseren")
    parser.add_argument("--optimize-metric", type=str, default="sharpe",
                       choices=["sharpe", "sortino", "calmar", "profit_factor", "win_rate", "max_drawdown"],
...
def run_backtest(df, strategy_params, risk_params, initial_capital=10000.0, detailed=False):
    """
    ...
    t0 = time.time()
    entry_signals, exit_signals = multi_layer_ema_strategy(
        df,
    ...
    if pnl_column and len(trades_df) > 0:
        ...
        results['avg_winning_trade'] = winning_trades[pnl_column].mean() if len(winning_trades) > 0 else 0
        results['avg_losing_trade'] = losing_trades[pnl_column].mean() if len(losing_trades) > 0 else 0
        results['best_trade'] = trades_df[pnl_column].max() if len(trades_df) > 0 else 0
        results['worst_trade'] = trades_df[pnl_column].min() if len(trades_df) > 0 else 0
    ...
    if detailed:
        ...
        try:
            ...
            results['ftmo_compliant'] = max_drawdown <= risk_params['max_total_loss'] and \
                                      max_daily_loss_pct >= -risk_params['max_daily_loss'] and \
            ...
            print(f"Aantal keer dagelijks verlies limiet doorbroken: {daily_loss_breaches}")
    ...
    print("\n" + "=" * 50)
    print(f"[⏱️] Backtest voltooid in {elapsed_time:.2f} seconden ({elapsed_time/60:.2f} minuten)")
    print("=" * 50)
...
def optimize_parameters(df, symbol, base_params, risk_params, param_to_optimize,
    ...
    """
    logger.info(f"Parameteroptimalisatie starten voor {param_to_optimize}, optimalisatie metric: {optimization_metric}")
    start_time = time.time()
    ...
    if param_to_optimize == "ema":
        ...
        total_combinations = sum(1 for ema1 in ema1_values
        ...
        for ema1 in ema1_values:
            for ema2 in ema2_values:
                for ema3 in ema3_values:
                    if ema1 < ema2 and ema2 < ema3:  # Zorg dat EMA's in oplopende volgorde zijn
                        ...
                        try:
                            ...
                            progress_bar.set_description(f"EMA {ema1}-{ema2}-{ema3}: Sharpe={sharpe_ratio:.2f}")

                        except Exception as e:
                            logger.error(f"Fout bij optimalisatie van EMA [{ema1}, {ema2}, {ema3}]: {str(e)}")
        ...
        # Bereken totaal aantal combinaties
        total_combinations = sum(1 for rsi_period in rsi_periods
                ...
                for overbought in overbought_values:
                    if oversold < overbought:  # Logische check
                        ...
                        try:
                            ...
                            # Update progress bar met beschrijving van huidige test
                            progress_bar.set_description(f"RSI {rsi_period} ({oversold}/{overbought}): Sharpe={sharpe_ratio:.2f}")

                        except Exception as e:
                            logger.error(f"Fout bij optimalisatie van RSI [period={rsi_period}, oversold={oversold}, overbought={overbought}]: {str(e)}")
        ...
        # Maak een tqdm progress bar
        progress_bar = tqdm(total=len(volatility_factors), desc="Optimalisatie voortgang",
                           bar_format='{l_bar}{bar:30}{r_bar}{bar:-30b}')
        ...
        for vol_factor in volatility_factors:
            ...
            try:
                ...
                # Update progress bar met beschrijving van huidige test
                progress_bar.set_description(f"Volatiliteit {vol_factor}: Sharpe={sharpe_ratio:.2f}")

            except Exception as e:
                logger.error(f"Fout bij optimalisatie van volatiliteitsfactor [{vol_factor}]: {str(e)}")
    ...
    if optimization_metric == 'sharpe':
        ...
        results_df['calmar'] = results_df['total_return'] / results_df['max_drawdown'].abs()
    ...
    elapsed_time = time.time() - start_time
    print("\n" + "=" * 60)
    print(f"[⏱️] Optimalisatie voltooid in {elapsed_time:.2f} seconden ({elapsed_time/60:.2f} minuten)")
    print("=" * 60)
        ...
        if hasattr(portfolio, 'entries') and hasattr(portfolio, 'exits'):
            ...
            for i in range(len(entries)):
                if i < len(df.index) and entries[i]:
                    ax1.scatter(df.index[i], df['close'].iloc[i], color='green', marker='^', s=100)
                if i < len(df.index) and exits[i]:
                    ax1.scatter(df.index[i], df['close'].iloc[i], color='red', marker='v', s=100)

        # Plot indicators
        if 'ema_short' in df.columns:
            ax1.plot(df.index, df['ema_short'], label=f"EMA kort", color='red', alpha=0.7)
        if 'ema_medium' in df.columns:
            ax1.plot(df.index, df['ema_medium'], label=f"EMA middel", color='blue', alpha=0.7)
        if 'ema_long' in df.columns:
            ax1.plot(df.index, df['ema_long'], label=f"EMA lang", color='black', alpha=0.7)
        if 'volatility_band' in df.columns:
            ax1.plot(df.index, df['volatility_band'], label="Volatility Band", color='purple', linestyle='--', alpha=0.7)
...
def main():
    ...
    logger.info(f"Backtesting {args.symbol} op {args.timeframe} van {start_date} tot {end_date}")
    ...
    t0 = time.time()
    df = get_data(args.symbol, args.timeframe, start_date, args.bars)
    t1 = time.time()
    print(f" Voltooid in {t1-t0:.2f} seconden")
    ...
    print(f"[✅] Data ontvangen: {len(df)} bars van {df.index[0].strftime('%Y-%m-%d %H:%M')} tot {df.index[-1].strftime('%Y-%m-%d %H:%M')}")
    ...
    if args.parameter_preset:
        ...
        asset_class = args.symbol  # get_strategy_params detecteert automatisch de asset class
    ...
    if args.optimize and args.optimize_param:
        strategy_params, best_params, results_df = optimize_parameters(
            df, args.symbol, strategy_params, risk_params,
            args.optimize_param, args.optimize_metric, args.capital
        )
    ...
    results, portfolio = run_backtest(df, strategy_params, risk_params, args.capital, args.detailed)
    ...
    print("\n" + "=" * 80)
    print(f"[⏱️] Totale uitvoeringstijd: {overall_elapsed_time:.2f} seconden ({overall_elapsed_time/60:.2f} minuten)")
    print("=" * 80)

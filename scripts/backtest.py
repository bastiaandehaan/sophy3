...
def parse_args():
    ...
    # Data argumenten
    parser.add_argument(
        "--symbol", type=str, default="EURUSD", help="Trading symbool (bijv. EURUSD)"
    )
    parser.add_argument(
        "--timeframe", type=str, default="H1", help="Timeframe (bijv. M15, H1, D1)"
    )
    ...
    parser.add_argument(
        "--bars", type=int, default=1000, help="Aantal bars voor backtesting"
    )
    parser.add_argument(
        "--no-cache", action="store_true", help="Data niet uit cache " "gebruiken"
    )
    parser.add_argument(
        "--refresh-cache",
        action="store_true",
    ...
    parser.add_argument(
        "--volatility-factor", type=float, help="Volatiliteit factor voor filter"
    )

    # Risk parameters
    parser.add_argument(
        "--capital", type=float, default=10000, help="Initieel kapitaal"
    )
    parser.add_argument(
        "--risk", type=float, default=0.01, help="Risico per trade (bijv. 0.01 voor 1%)"
    )
    parser.add_argument(
        "--max-daily-loss",
        type=float,
    ...
    # Output opties
    parser.add_argument(
        "--detailed", action="store_true", help="Toon gedetailleerde statistieken"
    )
    parser.add_argument("--plot", action="store_true", help="Toon grafieken")
    parser.add_argument(
        "--save-results", action="store_true", help="Sla resultaten op naar CSV"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
...
def run_backtest(
    df, strategy_params, risk_params, initial_capital=10000.0, detailed=False
):
    """
    ...
    if detailed:
        ...
        try:
            ...
            results["ftmo_compliant"] = (
                max_drawdown <= risk_params["max_total_loss"]
                and max_daily_loss_pct >= -risk_params["max_daily_loss"]
                and daily_loss_breaches == 0
            )
...
def optimize_parameters(
        ...
        for ema1 in ema1_values:
            for ema2 in ema2_values:
                for ema3 in ema3_values:
                    ...
                    ):  # Zorg dat EMA's in oplopende volgorde zijn
    ...
    if param_to_optimize == "ema":
        optimal_params["ema_periods"] = [
            int(best_params["ema1"]),
            int(best_params["ema2"]),
            int(best_params["ema3"]),
        ]
    elif param_to_optimize == "rsi":
...
def plot_backtest_results(df, portfolio, symbol, timeframe):
    ...
    try:
        ...
        fig, axs = plt.subplots(
            3, 1, figsize=(12, 16), gridspec_kw={"height_ratios": [2, 1, 1]}
        )
        ...
        if hasattr(portfolio, "entries") and hasattr(portfolio, "exits"):
            ...
            for i in range(len(entries)):
                ...
                if i < len(df.index) and exits[i]:
                    ax1.scatter(
                        df.index[i], df["close"].iloc[i], color="red", marker="v", s=100
                    )

        # Plot indicators
        if "ema_short" in df.columns:
            ax1.plot(
                df.index, df["ema_short"], label=f"EMA kort", color="red", alpha=0.7
            )
        if "ema_medium" in df.columns:
            ax1.plot(
                df.index, df["ema_medium"], label=f"EMA middel", color="blue", alpha=0.7
            )
        if "ema_long" in df.columns:
            ax1.plot(
                df.index, df["ema_long"], label=f"EMA lang", color="black", alpha=0.7
            )
...
def save_results(results, portfolio, symbol, timeframe, output_dir="./results"):
    ...
    try:
        ...
        results_file = os.path.join(
            output_dir, f"{symbol}_{timeframe}_{timestamp}_results.json"
        )
        ...
        try:
            ...
            trades_file = os.path.join(
                output_dir, f"{symbol}_{timeframe}_{timestamp}_trades.csv"
            )
...
def main():
    ...
    results, portfolio = run_backtest(
        df, strategy_params, risk_params, args.capital, args.detailed
    )

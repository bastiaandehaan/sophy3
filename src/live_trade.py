    ...
    try:
        ...
        if entries.iloc[-1] and not mt5.positions_get(symbol=symbol):
            ...
            size = risk_manager.calculate_position_size(
                current_price, stop_loss, risk_per_trade=0.01
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run live trading with Multi-Layer EMA strategy"
    )
    ...
    parser.add_argument(
        "--timeframe", type=str, default="H1", help="Timeframe (e.g. H1, D1)"
    )
    ...
    for symbol in active_symbols:
        if interval == "hour":
            schedule.every().hour.at(":01").do(
                check_for_signals, symbol=symbol, timeframe=args.timeframe
            )
        else:

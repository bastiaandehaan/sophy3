...
def get_strategy_params(asset_class: str) -> dict:
    ...
    if asset_class == 'forex':
        return {'ema_periods': [20, 50, 200], 'rsi_period': 14, 'rsi_oversold': 30,
                'rsi_overbought': 70, 'volatility_factor': 0.5}
    elif asset_class == 'crypto':
        return {'ema_periods': [10, 20, 50], 'rsi_period': 14, 'rsi_oversold': 40,
                'rsi_overbought': 60, 'volatility_factor': 1.5}
    elif asset_class == 'index':
        return {'ema_periods': [10, 30, 100], 'rsi_period': 14, 'rsi_oversold': 40,
                'rsi_overbought': 60, 'volatility_factor': 0.75}
    else:  # stocks
        return {'ema_periods': [9, 21, 50], 'rsi_period': 14, 'rsi_oversold': 30,
                'rsi_overbought': 70, 'volatility_factor': 0.5}
...
def get_risk_params(asset_class: str) -> dict:
    ...
    """
    risk_params = {'max_daily_loss': 0.05, 'max_total_loss': 0.10,
                   'profit_target': 0.10, 'min_trading_days': 4}
...
def get_default_params():
    ...
    """
    return {'ema_periods': [20, 50, 200], 'rsi_period': 14, 'rsi_oversold': 30,
            'rsi_overbought': 70, 'volatility_factor': 1.0, 'stop_loss_atr_multiple': 2.0,
            'take_profit_atr_multiple': 3.0, 'atr_period': 14, 'trailing_stop': True,
            'trailing_stop_activation': 0.5,  # % of take profit
            'max_spread_points': 10, 'max_daily_trades': 5}